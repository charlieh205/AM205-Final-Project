...
def xpp(s,i, num_bodies, G):
    other_indices = list(range(num_bodies))
    other_indices.remove(i)
    return G*sum([x_accel(s,i,j,num_bodies) for j in other_indices])

def ypp(s,i, num_bodies, G):
    other_indices = list(range(num_bodies))
    other_indices.remove(i)
    return G*sum([y_accel(s,i,j,num_bodies) for j in other_indices])

def zpp(s,i, num_bodies, G):
    other_indices = list(range(num_bodies))
    other_indices.remove(i)
    return G*sum([z_accel(s,i,j,num_bodies) for j in other_indices])

def F(s,t, G=1):
    num_bodies = int(s.shape[0]/6)
    return np.concatenate(
        (s[3*num_bodies:4*num_bodies],s[4*num_bodies:5*num_bodies],s[5*num_bodies:6*num_bodies],
         [xpp(s,i, num_bodies, G) for i in range(num_bodies)],
         [ypp(s,i, num_bodies, G) for i in range(num_bodies)],
         [zpp(s,i,num_bodies, G) for i in range(num_bodies)]))
