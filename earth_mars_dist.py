...
def main(start_date, n_years):
    ...
    xtick_labs = [
        d.split(".")[0]
        for d in ["-".join([str(a) for a in mjd_to_date(x)]) for x in xticks]
    ]
    ...
    plt.plot(
        ...
        "o",
        color="red",
        label=f"Date of shortest distance:\n{min_date}",
    )
    ax = plt.gca()
    ...
    plt.title(
        f"Distance between Earth and Mars over the next {n_years} years", fontsize=20
    )
